...
export default function KelolaUser() {
      ...
      useEffect(() => {
            if (isLoggedIn && (isAdmin || isSuperAdmin)) {
                  ...
                  // Auto refresh blocked status setiap 30 detik
      ...
      const fetchBlockedStatus = async () => {
            ...
            try {
                  ...
                  if (err.response?.status === 401) {
                        console.log("Unauthorized access to blocked status - user might not be admin");
                        setBlockedStatus({});
                        ...
                        console.log("Forbidden access to blocked status");
                        setBlockedStatus({});
                  } else {
                        setBlockedStatus({});
                  }
...
const renderBlockedStatus = (userId) => {
      ...
      if (blocked) {
            ...
            return (
                  <Tooltip label={`Diblokir sampai: ${blockInfo?.resetTime.toLocaleString()}\nSisa waktu: ${blockInfo?.timeLeftFormatted}\nAlasan: ${blockInfo?.type || 'N/A'}`}>
