        ...
        <table className="w-full text-sm border mb-4">
          <thead>
            <tr className="bg-gray-100 dark:bg-gray-800">
              ...
              <th className="px-2 py-1 border">Nama Material</th>
              {hasBatch && <th className="px-2 py-1 border">No Batch</th>}
            </tr>
          </thead>
          <tbody>
...
const EnhancedPendingBox = ({ label, value, icon: Icon, color, onClick }) => {
  ...
  return (
    <div
      className={`${getColorClasses(color)} rounded-lg p-4 border shadow-sm hover:shadow-md transition-all duration-300 hover:scale-105 cursor-pointer`}
      onClick={onClick}
    >
...
export default function SampleMonitoringDashboard() {
  ...
  // State untuk modal tabel material
  ...
  return (
    <div className={`min-h-screen ${bg} ${textColor} p-4 mt-20 w-full`}>
            ...
            <div className="flex items-center justify-center mb-2">
              ...
              <span className="text-sm font-medium text-blue-600 cursor-pointer"
                onClick={() => setDetailModal({
                  open: true,
                  title: "RM Sample Received (last 2 days)",
                  data: parseMaterialList(qcData.daftar_kedatangan?.kedatangan_rm)
                })}
              >
                RM Samples Received (Last 2 Days)
              </span>
            </div>
            <div
          ...
          <div className="text-center">
            <div className="flex items-center justify-center mb-2">
              ...
              <span className="text-sm font-medium text-purple-600 cursor-pointer"
                onClick={() => setDetailModal({
                  open: true,
                  title: "PM Sample Received (last 2 days)",
                  data: parseMaterialList(qcData.daftar_kedatangan?.kedatangan_pm)
                })}
              >
                PM Samples Received (Last 2 Days)
              </span>
            </div>
            <div
            ...
            <div className={`${boxBg} rounded-xl shadow-xl p-6 mb-8 border ${borderColor} transition-all duration-500`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <EnhancedPendingBox
                  ...
                  value={qcData.total_sample_qc.pending_sampling}
                  icon={Clock}
                  color="orange"
                  onClick={() => setDetailModal({
                    open: true,
                    title: "Pending Sampling (All)",
                    data: parseMaterialList(qcData.daftar_pendingan_all?.pending_sampling)
                  })}
                />
                <EnhancedPendingBox
                  label="Pending Analysis"
                  value={qcData.total_sample_qc.pending_analisa}
                  icon={Activity}
                  color="blue"
                  onClick={() => setDetailModal({
                    open: true,
                    title: "Pending Analysis (All)",
                    data: parseMaterialList(qcData.daftar_pendingan_all?.pending_analisa)
                  })}
                />
                <EnhancedPendingBox
                  label="Pending SPV Approval"
                  value={qcData.total_sample_qc.pending_approval_spv}
                  icon={Users}
                  color="purple"
                  onClick={() => setDetailModal({
                    open: true,
                    title: "Pending SPV Approval (All)",
                    data: parseMaterialList(qcData.daftar_pendingan_all?.pending_approval_spv)
                  })}
                />
                <EnhancedPendingBox
                  label="Pending Admin Check"
                  value={qcData.total_sample_qc.pending_admin}
                  icon={FileText}
                  color="indigo"
                  onClick={() => setDetailModal({
                    open: true,
                    title: "Pending Admin Check (All)",
                    data: parseMaterialList(qcData.daftar_pendingan_all?.pending_admin)
                  })}
                />
                <EnhancedPendingBox
                  label="Pending Manager Release"
                  value={qcData.total_sample_qc.pending_approval_mgr}
                  icon={Target}
                  color="green"
                  onClick={() => setDetailModal({
                    open: true,
                    title: "Pending Manager Release (All)",
                    data: parseMaterialList(qcData.daftar_pendingan_all?.pending_approval_mgr)
                  })}
                />
              </div>
            </div>
            ...
            {showPendingDetailsRM && (
              <div className={`${boxBg} rounded-xl shadow-xl p-6 mb-4 border ${borderColor} transition-all duration-500`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  <EnhancedPendingBox
                    label="Pending Sampling"
                    value={qcData.total_sample_qc_rm.pending_sampling_rm}
                    icon={Clock}
                    color="orange"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending Sampling RM",
                      data: parseMaterialList(qcData.daftar_pendingan_rm?.pending_sampling_rm)
                    })}
                  />
                  <EnhancedPendingBox
                    label="Pending Analysis"
                    value={qcData.total_sample_qc_rm.pending_analisa_rm}
                    icon={Activity}
                    color="blue"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending Analysis RM",
                      data: parseMaterialList(qcData.daftar_pendingan_rm?.pending_analisa_rm)
                    })}
                  />
                  <EnhancedPendingBox
                    label="Pending SPV Approval"
                    value={qcData.total_sample_qc_rm.pending_approval_spv_rm}
                    icon={Users}
                    color="purple"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending SPV Approval RM",
                      data: parseMaterialList(qcData.daftar_pendingan_rm?.pending_approval_spv_rm)
                    })}
                  />
                  <EnhancedPendingBox
                    label="Pending Admin Check"
                    value={qcData.total_sample_qc_rm.pending_admin_rm}
                    icon={FileText}
                    color="indigo"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending Admin Check RM",
                      data: parseMaterialList(qcData.daftar_pendingan_rm?.pending_admin_rm)
                    })}
                  />
                  <EnhancedPendingBox
                    label="Pending Manager Release"
                    value={qcData.total_sample_qc_rm.pending_approval_mgr_rm}
                    icon={Target}
                    color="green"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending Manager Release RM",
                      data: parseMaterialList(qcData.daftar_pendingan_rm?.pending_approval_mgr_rm)
                    })}
                  />
                </div>
              </div>
            ...
            {showPendingDetailsPM && (
              <div className={`${boxBg} rounded-xl shadow-xl p-6 mb-4 border ${borderColor} transition-all duration-500`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  <EnhancedPendingBox
                    label="Pending Sampling"
                    value={qcData.total_sample_qc_pm.pending_sampling_pm}
                    icon={Clock}
                    color="orange"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending Sampling PM",
                      data: parseMaterialList(qcData.daftar_pendingan_pm?.pending_sampling_pm)
                    })}
                  />
                  <EnhancedPendingBox
                    label="Pending Analysis"
                    value={qcData.total_sample_qc_pm.pending_analisa_pm}
                    icon={Activity}
                    color="blue"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending Analysis PM",
                      data: parseMaterialList(qcData.daftar_pendingan_pm?.pending_analisa_pm)
                    })}
                  />
                  <EnhancedPendingBox
                    label="Pending SPV Approval"
                    value={qcData.total_sample_qc_pm.pending_approval_spv_pm}
                    icon={Users}
                    color="purple"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending SPV Approval PM",
                      data: parseMaterialList(qcData.daftar_pendingan_pm?.pending_approval_spv_pm)
                    })}
                  />
                  <EnhancedPendingBox
                    label="Pending Admin Check"
                    value={qcData.total_sample_qc_pm.pending_admin_pm}
                    icon={FileText}
                    color="indigo"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending Admin Check PM",
                      data: parseMaterialList(qcData.daftar_pendingan_pm?.pending_admin_pm)
                    })}
                  />
                  <EnhancedPendingBox
                    label="Pending Manager Release"
                    value={qcData.total_sample_qc_pm.pending_approval_mgr_pm}
                    icon={Target}
                    color="green"
                    onClick={() => setDetailModal({
                      open: true,
                      title: "Pending Manager Release PM",
                      data: parseMaterialList(qcData.daftar_pendingan_pm?.pending_approval_mgr_pm)
                    })}
                  />
                </div>
              </div>
      ...
      {/* Modal Material List */}
      <ModalMaterialList {...detailModal} onClose={() => setDetailModal({ ...detailModal, open: false })} />
