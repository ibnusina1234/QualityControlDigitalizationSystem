...
const {dynamicRateLimiter,loginRateLimiter} = require("../middleware/rateLimit");
...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    const safeName = itemType && itemId
...
const fileFilter = (req, file, cb) => {
  ...
  if (ALLOWED_EXTENSIONS.test(ext) && ALLOWED_MIME_TYPES.includes(mime)) {
    ...
    cb(new Error("Hanya file gambar dengan format jpg, jpeg, png, webp yang diperbolehkan."));
  }
};
...
router.post("/Register",dynamicRateLimiter,upload.single("img"),validate(registerSchema),sanitizeInput, userController.registerUser);
...
router.post(
  ...
  [
    ...
    body('password').notEmpty().withMessage('Password wajib diisi'),validatePassword,
    validate(loginSchema),sanitizeInput,
  ],
  userController.loginUser
);

// 🔹 Logout pengguna
router.post("/logout", dynamicRateLimiter,verifyToken,updateTokenExpiry, userController.logoutUser);
...
//🔹 update token
router.post("/users/updateToken", verifyToken, updateTokenExpiry, (req, res) => {
...
router.get("/Profile",verifyToken,updateTokenExpiry,userController.getProfile);

// 🔹 Memperbarui profil pengguna
router.put("/profile", dynamicRateLimiter,verifyToken,updateTokenExpiry,upload.single("img"),  userController.updateProfile);

router.put("/userrole/:id", dynamicRateLimiter,verifyToken, userController.updateUserRole);

// 🔹 Permintaan reset password
router.post("/request-password-reset", dynamicRateLimiter,userController.requestPasswordReset);

// 🔹 Reset password
router.post("/reset-password", dynamicRateLimiter,sanitizeInput,userController.resetPassword);
...
// 🔹 Menghapus user
router.delete("/deleteUser/:id",dynamicRateLimiter,verifyToken, updateTokenExpiry,authorizeRoles("admin","super admin"), userController.deleteUserById);

// 🔹 Menyetujui atau menolak pengguna
router.patch("/updateUserStatus/:id", dynamicRateLimiter,verifyToken, updateTokenExpiry,authorizeRoles("admin","super admin"),async (req, res) => {
      ...
      try {
            // Ambil data user
            const [userRows] = await db.query("SELECT email, nama_lengkap FROM user WHERE id = ?", [id]);
