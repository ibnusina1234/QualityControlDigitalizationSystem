...
const SHEETS = ['total_sample_qc', 'total_sample_qc_rm', 'total_sample_qc_pm','daftar_kedatangan','daftar_pendingan_pm','daftar_pendingan_rm'];
    ...
    const data = await Promise.all(
      SHEETS.map(async (name) => {
        const result = await sheets.spreadsheets.values.get({
          ...
          range: `${name}!A1:U3`, // Ambil header dan baris data
        });
