      ...
      const { material_name, operator_id, batch_number, vat_count, requested_at } = req.body;
      try {
      let [material] = await db.query(
      ...
      console.log('DATA AKAN DISIMPAN', { batch_id, material_id, operator_id, requested_at, });
      const [result] = await db.query(
      ...
      const { request_id } = req.params;
      try {
      await db.query('DELETE FROM request_vats WHERE request_id = ?', [request_id]);
      await db.query('DELETE FROM raman_requests WHERE id = ?', [request_id]);
      ...
      try {
      const [reqRows] = await db.query(`SELECT batch_id FROM raman_requests WHERE id = ?`, [request_id]);
      ...
      try {
      await db.query(`UPDATE raman_requests SET ${field} = ? WHERE id = ?`, [
            new_value,
            request_id,
      ]);
      await db.query(
